...
export function NewsletterConfirm() {
  ...
  return (
    <main className="mx-auto flex min-h-[60vh] max-w-page items-center justify-center bg-white px-5 py-16 text-center text-ink lg:px-8">
      {status === 'loading' ?
      <LoadingBanner label="Confirming your subscription…" /> :
      status === 'ok' ?
      <div>
          <h1 className="text-3xl font-bold tracking-tight text-ink">You're subscribed 🎉</h1>
          <p className="mt-2 text-[14.5px] text-neutral-600">
            You'll get the newsletter at your inbox from now on.
          </p>
        </div> :
